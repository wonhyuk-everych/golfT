...
// Touch events (for browsers that don't unify to pointer events)
...
// Thumbnails: globally limit when more than 5 images and show "+N" tile
const MAX_THUMBS = 5

const displayThumbs = computed(() => {
  if (!Array.isArray(props.images)) return []
  if (props.images.length > MAX_THUMBS) {
    // Show first (MAX-1) thumbnails; last tile becomes +N
...
const nextHiddenIndex = computed(() => {
  // The first hidden index
  return displayThumbs.value.length
})
