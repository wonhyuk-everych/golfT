<template>
  <div class="reservation-container p-4">
    ...
    <!-- 환불 불가 알림 (조건부 표시) -->
    <div
      v-if="selectedRoom && selectedRoom.refund_yn === 'N'"
    ...
    <div class="check-dates bg-gray-50 p-4 rounded-md mb-4">
      <div class="flex justify-between mb-2">
        ...
        <div>
          ...
          <p class="font-medium">
            {{ formatDate(checkOutDate, locale.value) }}
          </p>
        </div>
      </div>
    ...
    <div class="room-info p-4 mb-4 flex flex-col gap-6 px-4">
      ...
      <div class="flex flex-col gap-1">
        ...
        <span v-if="children > 0" class="text-text-secondary text-sm"
          >아이 {{ children }}명</span
        >
        <span class="text-red-500 text-sm"
          >* 예약 인원과 실제 투숙 인원이 다를 경우,<br />
    ...
    <div v-if="paidServices.length > 0" class="additional-info mb-6 p-4">
      ...
      <div class="flex flex-col gap-2">
        <label
          ...
          :key="service.hotel_paid_service_idx"
        ...
        >
          ...
          <span>
            {{ service.service_name }} ({{
              formatPriceWithRate(service.price, locale)
            }})
          </span>
        </label>
      </div>
    ...
    <div class="flex flex-col gap-2 mb-6 p-4">
      ...
      <div class="flex flex-col gap-2">
        <div class="flex flex-row justify-between items-center">
          <span class="text-text-secondary text-sm"
            >총 객실 금액 ({{ roomCount }}개 x {{ calculateNights() }}박)</span
          >
          <span class="text-text-secondary text-sm">{{
            formatPriceWithRate(totalRoomPrice, locale)
          }}</span>
        </div>
        <div
          v-if="selectedPaidServices.length > 0"
          class="flex flex-row justify-between items-center"
        >
          <span class="text-text-secondary text-sm"
            >추가 유료 옵션 ({{ selectedPaidServices.length }}개)</span
          >
          <span class="text-text-secondary text-sm">{{
            formatPriceWithRate(selectedPaidServicesTotal, locale)
          }}</span>
        </div>
        <div
        ...
        >
          ...
          <span class="text-text-secondary text-sm">{{
            formatPriceWithRate(discountAmount, locale)
          }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col pb-6">
      <div class="flex flex-row justify-between items-center mb-2">
        ...
        <span class="font-bold text-xl text-primary">{{
          formatPriceWithRate(finalPaymentAmount, locale)
        }}</span>
      </div>
    </div>
...
const selectedPaidServicesTotal = computed(() => {
  ...
  return selectedPaidServices.value.reduce((sum: number, idx: number) => {
    const found = props.paidServices.find(
      (s: HotelPaidService) => s.hotel_paid_service_idx === idx
    );
...
const finalPaymentAmount = computed(() => {
  return (
    ...
    selectedPaidServicesTotal.value -
...
const handleAddCart = async () => {
  ...
  try {
    const response = await $fetch("/api/hotel/add-cart", {
      ...
      body: JSON.stringify(hotel), // JSON.stringify -> 시간객체는 자동으로 UTC변환 처리를 해줌
    });
