...
google map 개발 상황
 - 심사 실패 이유
  : google-feeds 형식에 적절치 않은 url
     ( google-feeds 를 가지고 구글맵에서 redirect 해주는 용도 )
  : 예약 링크를 처음 띄웠을 떄 골프장 이름,주소,위치 정보가 바로 나오지 않음
  : Brand 등록할 때 기입안된 부분이 있었음

 - 구글 정책 요구사항 부족한 부분 ( 요청 오면 하는거로 )
  : 비로그인 예약 정책 추가
  : 구글맵 예약 전용 페이지 추가 ( 이름,주소,위치 상단에 고정 )
  : 비로그인 예약시 회원 정보 입력 페이지 