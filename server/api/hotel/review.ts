...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    let queryString = `
      ...
      JOIN hotel G ON R.product_idx = G.hotel_idx
      WHERE R.review_type = 'H' AND R.use_yn = 'Y'
    `
    
    // product_idx가 있으면 필터링 조건 추가
    if (productIdx) {
      queryString += ` AND G.hotel_idx = ?`
    }
    ...
    const [totalRows] = await pool.query(`
      ...
      FROM review R
      JOIN member M ON R.member_idx = M.member_idx
      JOIN hotel H ON R.product_idx = H.hotel_idx
      WHERE R.product_idx = ? AND R.review_type = 'H' AND R.use_yn = 'Y'
    `,[productIdx])
