...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    let queryString = `
      ...
      JOIN caddy C ON R.product_idx = C.caddy_idx
      WHERE R.review_type = 'C' AND R.use_yn = 'Y'
    `
    
    // product_idx가 있으면 필터링 조건 추가
    if (productIdx) {
      queryString += ` AND C.caddy_idx = ?`
    }
    ...
    const [totalRows] = await pool.query(`
      ...
      JOIN member M ON R.member_idx = M.member_idx
      JOIN caddy C ON R.product_idx = C.caddy_idx
      WHERE R.product_idx = ? AND R.review_type = 'C' AND R.use_yn = 'Y'
    `,[productIdx])
